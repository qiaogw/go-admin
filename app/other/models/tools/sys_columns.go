...
func (e *SysColumns) GetList(tx *gorm.DB, exclude bool) ([]SysColumns, error) {
	var doc []SysColumns
	table := tx.Table(global.TablePrefix + "sys_columns")
...
func (e *SysColumns) Create(tx *gorm.DB) (SysColumns, error) {
	var doc SysColumns
	e.CreateBy = 0
	result := tx.Table(global.TablePrefix + "sys_columns").Create(&e)
	if result.Error != nil {
		err := result.Error
		return doc, err
	}
...
func (e *SysColumns) Update(tx *gorm.DB) (update SysColumns, err error) {
	if err = tx.Table(global.TablePrefix+"sys_columns").First(&update, e.ColumnId).Error; err != nil {
		return
	}
	...
	//参数2:是修改的数据
	e.UpdateBy = 0
	if err = tx.Table(global.TablePrefix + "sys_columns").Model(&update).Updates(&e).Error; err != nil {
		return
	}
